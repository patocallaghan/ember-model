...
test("relationship should be refreshed when data changes", function(assert) {
  var Article = Ember.Model.extend({
        slug: Ember.attr(String)
      }),
      Comment = Ember.Model.extend({
...
test("when fetching an association getBelongsTo is called", function(assert) {
  assert.expect(4);
...
test("toJSON uses the given relationship key in belongsTo", function(assert) {
  assert.expect(1);
...
test("un-embedded belongsTo CP should handle set", function(assert) {
  assert.expect(1);
...
test("embedded belongsTo CP should handle set", function(assert) {
  assert.expect(1);
...
test("must be set with value of same type", function(assert) {
  assert.expect(1);
...
test("should be able to set embedded relationship to null", function(assert) {
  assert.expect(2);
...
test("should be able to set nonembedded relationship to null", function(assert) {
  assert.expect(2);
...
test("materializing the relationship should should not dirty the record", function(assert) {
  assert.expect(2);
...
test("setting relationship should make parent dirty", function(assert) {
  assert.expect(1);
...
test("setting existing nonembedded relationship should make parent dirty", function(assert) {
  assert.expect(1);
...
test("setting existing nonembedded relationship to NULL should make parent dirty", function(assert) {
  assert.expect(1);
...
test("relationships should be seralized when specified with string", function(assert) {
  assert.expect(1);
...
test("belongsTo from an embedded source is able to materialize without having to re-find", function(assert) {
...
test("unloaded records are removed from reference cache", function(assert) {
...
test("unloaded records are removed from hasMany cache", function(assert) {
  var Company = Ember.Company = Ember.Model.extend({
     id: Ember.attr('string'),
     title: Ember.attr('string'),
     projects: Ember.hasMany('Ember.Project', {key:'projects', embedded: true})
...
test("belongsTo records created are available from reference cache", function(assert) {
...
test("embedded belongsTo with undefined value", function(assert) {
  assert.expect(1);
  var json = {
    id: 1,
    name: 'foo'
