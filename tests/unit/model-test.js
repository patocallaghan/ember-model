...
test("creates reference when creating record", function(assert) {
  assert.expect(4);
...
test("updates reference and cache when primary key changes", function(assert) {
  assert.expect(7);
...
test("isLoaded observers have all the updated properties", function(assert) {
  assert.expect(2);
...
test("can define attributes with Ember.attr, data is accessible", function(assert) {
  var instance = Model.create({name: "Erik"});

  assert.equal(instance.get('name'), "Erik", "Property value was retained");
});

test("can handle models without an ID", function(assert) {
  assert.expect(3);
...
test("can handle models with ID of zero", function(assert) {
  assert.expect(1);
...
test(".find([]) called with a single record returns cache before delgating to adapter's find method", function(assert) {
  assert.expect(1);
...
test(".find([]) called when Model.transient true always delegates to adapter's find", function(assert) {
  assert.expect(3);
...
test(".reload() loads the record via the adapter after it was loaded", function(assert) {
  assert.expect(1);
...
test(".revert() works on new records with no attributes", function(assert) {
  assert.expect(4);
...
test("new records are added to the identity map", function(assert) {
  assert.expect(2);
...
test("creating a new record adds it to existing record arrays", function(assert) {
  assert.expect(1);
...
test("destroying a record removes it from record arrays", function(assert) {
  assert.expect(2);
...
test("record isNew & isSaving flags", function(assert) {
  assert.expect(5);
...
test("record.toJSON() uses rootKey if it is defined", function(assert) {
  assert.expect(1);
...
test("record.toJSON() can use computed property as rootKey", function(assert) {
  assert.expect(1);
...
test("Model#create() works as expected", function(assert) {
  assert.expect(10);
...
test(".getAttributes() returns the model's attributes", function(assert) {
  var attr = Ember.attr,
...
test(".getRelationships() returns the model's relationships", function(assert) {
...
test("toJSON includes embedded relationships", function(assert) {
  var attr = Ember.attr,
...
test("toJSON includes non-embedded relationships", function(assert) {
  var Comment = Ember.Model.extend({
        id: Ember.attr(),
        text: Ember.attr()
      }),
...
test("creating a record with camelizedKeys = true works as expected", function(assert) {
  assert.expect(1);
...
test("can use data as attribute name", function(assert) {
  assert.expect(1);
...
test("record is available in reference cache when load is run in cachedRecordForId", function(assert) {
  var recordFromCache,
...
test("fetchQuery returns a promise", function(assert) {
  assert.expect(1);
...
test("fetchMany returns a promise", function(assert) {
  var FixtureFindQueryAdapter = Ember.FixtureAdapter.extend({
...
test("fetchQuery resolves with error object", function(assert) {
  assert.expect(1);
...
test("fetchAll resolves with error object", function(assert) {
  assert.expect(1);
...
test("fetchById resolves with error object", function(assert) {
  assert.expect(1);
...
// });
test(".clearCache destroys _findAllRecordArray reference", function(assert) {
  assert.expect(1);
