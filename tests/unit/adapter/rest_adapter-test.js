...
test("throws an error if a url isn't provided", function(assert) {
  assert.expect(3);

  assert.throws(function() {
    run(RESTModel, RESTModel.find);
  }, /requires a `url` property to be specified/);

  assert.throws(function() {
    run(RESTModel, RESTModel.find, 1);
  }, /requires a `url` property to be specified/);
...
test("findAll loads the full JSON payload when collectionKey isn't specified", function(assert) {
  assert.expect(1);
...
test("findAll loads the proper JSON payload subset when collectionKey is specified", function(assert) {
  assert.expect(1);
...
test("findAll uses Ember.get for a collectionKey", function(assert) {
  assert.expect(1);

  RESTModel.reopenClass({
    collectionKey: computed(function() {
      return 'posts';
    })
  });
...
test("findAll calls didFindAll callback after finishing", function(assert) {
  assert.expect(4);
...
test("findById loads the full JSON payload when rootKey isn't specified", function(assert) {
  assert.expect(1);
...
test("findById loads the proper JSON payload subset when rootKey is specified", function(assert) {
  assert.expect(1);
...
test("findById uses Ember.get to fetch rootKey", function(assert) {
  assert.expect(1);
...
test("find calls didFind after finishing", function(assert) {
  assert.expect(4);
...
test("findQuery", function(assert) {
  assert.expect(3);
  ...
  run(RESTModel, RESTModel.find, {foo: 'bar'});
});

test("findQuery loads the full JSON payload when collectionKey isn't specified", function(assert) {
  assert.expect(1);
...
test("findQuery loads the data from a specified collectionKey", function(assert) {
  assert.expect(1);
...
test("findQuery uses Ember.get for a collectionKey", function(assert) {
  assert.expect(1);
...
test("findQuery calls didFindQuery callback after finishing", function(assert) {
  assert.expect(4);
...
test("findQuery with params", function(assert) {
  assert.expect(1);
  ...
  run(RESTModel, RESTModel.find, {foo: 'bar', num: 42});
});
...
test("createRecord", function(assert) {
  ...
  run(record, record.save);

  assert.ok(!record.get('isNew'), "Record should not be new");
});

test("createRecord calls didCreateRecord", function(assert) {
  assert.expect(5);
...
test("createRecord record loads data in response", function(assert) {
  assert.expect(2);
...
test("saveRecord", function(assert) {
  ...
  var record = run(RESTModel, RESTModel.create, {id: 1, name: "Erik", isNew: false});
...
test("saveRecord calls didSaveRecord after saving record", function(assert) {
  assert.expect(5);
...
test("saveRecord loads response data if it exists", function(assert) {
  assert.expect(4);
...
test("saveRecord does not load empty response", function(assert) {
  assert.expect(4);
  var record = run(RESTModel, RESTModel.create, {id: 1, name: "Erik", isNew: false}),
      responseData = '';
...
test("saveRecord does not load HEAD response (undefined response body)", function(assert) {
  assert.expect(4);
  var record = run(RESTModel, RESTModel.create, {id: 1, name: "Erik", isNew: false}),
      responseData = '';
...
test("saveRecord does not load response if root key is missing", function(assert) {
  assert.expect(4);
  var record = run(RESTModel, RESTModel.create, {id: 1, name: "Erik", isNew: false}),
...
test("deleteRecord", function(assert) {
  ...
  run(record, record.deleteRecord);

  assert.ok(record.get('isDeleted'), "Record should be deleted");
});

test("deleteRecord calls didDeleteRecord after deleting", function(assert) {
  assert.expect(5);
...
test("Model.find([id]) works as expected", function(assert) {
  assert.expect(1);
  ...
  record = run(RESTModel, RESTModel.find, 1);
  records = run(RESTModel, RESTModel.find, [1]);

  promise = run(RSVP, all, [records, record]);

  run(promise, promise.then, function() {
...
test("Expect ajax settings to include a custom header", function(assert) {
  var settings = run(RESTModel.adapter, RESTModel.adapter.ajaxSettings, RESTModel.url, "GET");
...
test("Expect ajax settings to include a custom header", function(assert) {
  var settings = run(adapter, adapter.ajaxSettings, RESTModel.url, "GET");
  assert.equal(settings.headers.authentication, "xxx-yyy");
  assert.equal(settings.type, "GET");
  assert.equal(settings.url, RESTModel.url);
...
test("find with 0", function(assert) {
  assert.expect(3);
  ...
  run(RESTModel, RESTModel.find, 0);
});

test("find() resolves with record", function(assert) {
  assert.expect(1);
...
test("findAll() resolves with records", function(assert) {
  assert.expect(1);
...
test("findQuery() resolves with records", function(assert) {
  assert.expect(1);
...
test("createRecord() resolves with record", function(assert) {
  assert.expect(1);
...
test("saveRecord() resolves with record", function(assert) {
  assert.expect(1);
...
test("buildURL() creates url from model's url, id, and url suffix", function(assert) {
  assert.expect(3);
  RESTModel.url = "/posts";
...
test('_handleRejections() will resolve empty successful DELETEs', function(assert) {
  assert.expect(1);
  var resolve = function(data) {
    assert.ok(data === null, "resolved with null");
  };
  run(function() {
    adapter._handleRejections("DELETE", {status: 200}, resolve, $.noop);
  });
});

test('_handleRejections() will reject empty responses for other verbs', function(assert) {
  assert.expect(1);
  var reject = function(data) {
    assert.ok(data.status === 200, "rejected with 200 status");
  };
  run(function() {
    adapter._handleRejections("PUT", {status: 200}, $.noop, reject);
  });
});

test('_handleRejections() will reject DELETEs with unsuccessful status codes', function(assert) {
  assert.expect(1);
  var reject = function(data) {
    assert.ok(data.status === 403, "rejected with 403 status");
  };
  run(function() {
    adapter._handleRejections("DELETE", {status: 403}, $.noop, reject);
  });
});
